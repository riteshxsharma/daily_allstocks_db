...
def execute_query():
    conditions = []
    for i in range(5):
        column = column_vars[i].get()
        operator = operator_vars[i].get()
    ...
    columns_to_return = ["Order", "Symbol", "Name", "Current Price", "Price % Chg", "% Chg YTD",
                         "% Chg Cur Week", "% Chg 1 Month", "Volume (1000s)", "Vol % Chg vs 10-Week"]
    columns_string = ", ".join([f'"{col}"' for col in columns_to_return])

    # Create the query string
    query = f'SELECT {columns_string} FROM all_stocks_20240903pm15 WHERE {" AND ".join(conditions)}'
    try:
        ...
        column_widths = [20] * len(columns_to_return)  # Specify the width for each column
